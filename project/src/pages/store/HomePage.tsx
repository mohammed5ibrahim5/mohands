...
export default function HomePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setBestsellers((feats || []).slice(0, 4));
      setLoading(false);
    })();
  }, []);
    ...
    <div className="animate-fade-in">
      ...
      <section className="relative overflow-hidden bg-stone-950 text-white">
        <div className="absolute inset-0">
          <img src="https://images.pexels.com/photos/6340707/pexels-photo-6340707.jpeg?auto=compress&cs=tinysrgb&w=1200" alt="" className="w-full h-full object-cover opacity-20" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-36">
          <div className="max-w-2xl">
            ...
            <p className="text-stone-300 text-lg md:text-xl mt-8 leading-relaxed max-w-xl">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {categories.map((cat) => (
            <Link key={cat.id} to={`/shop?category=${cat.slug}`} className="group relative aspect-[3/4] rounded-2xl overflow-hidden card hover:shadow-xl transition-all duration-500 hover:-translate-y-1">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-l from-stone-900 to-stone-800 p-10 md:p-16">
          ...
          <div className="relative grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div>
              <span className="inline-block bg-amber-500 text-stone-900 text-sm font-bold px-4 py-1.5 rounded-full mb-4">عرض حصري</span>
              <h2 className="font-serif text-3xl md:text-4xl font-bold text-white mb-4">خصومات تصل إلى 40% على الأدوات المكتبية</h2>
              <p className="text-stone-300 text-lg mb-6">لفترة محدودة فقط! استمتع بخصومات حصرية على تشكيلة واسعة من منتجاتنا المميزة.</p>
              <Link to="/shop?sort=price-low" className="btn-accent">اطلب الآن قبل نفاد الكمية</Link>
            </div>
            <div className="hidden md:block text-center">
              <p className="font-serif text-8xl font-bold text-amber-500/30">40%</p>
            ...
            {testimonials.map((t, i) => (
              <div key={i} className="card p-8 relative">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-stone-800 flex items-center justify-center text-amber-400 font-bold text-lg">
                    {t.name.charAt(0)}
                  </div>
                  <div>
